...
\label{ass-grammar}
The fist assumption we are going to introduce involves restrictions on the emission protocol of an \code{Observable}. Events propagated to the \code{Observer} continuation will obey the following regular expression:

\begin{center}\code{onNext* (onError | onCompleted)?}\end{center}

This grammar allows streams to propagate any number - 0 or more - of events though the \code{onNext} function, optionally followed by a message indicating termination, be that natural - though \code{onCompleted} - or due to a failure - though \code{onError}. Note how the optional nature of a termination message allows for infinite streams. 
