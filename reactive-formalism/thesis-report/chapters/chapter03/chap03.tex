...
The following outlines the chapter and helps clarifying the steps we will undertake in the remainder of our discussion: first, think back to Section \ref{sec:applyingduality}, when we compared \code{Observable} to a setter of setters, the result of applying the \code{Observer} function to itself. Starting from this notion let's define a function \code{(!)}, representing an \code{Observer}:\todo{Should I make them datatype?}

\hmint |!a :: a -> IO () |

When we apply the function to itself we obtain the \code{Observable}, which, as we saw previously, has the same signature and can be implemented by the continuation monad.

\hmint |!!a :: (a -> IO ()) -> IO () |

next, we are going to define two new functions, \code{(?)}, representing a continuation that can deal with termination and errors as inputs, and \code{(\%)}, a function handing over a \code{Subscription} which will be used to implement premature stream termination.
...
In this chapter we are going to modify from our initial continuation-based definition of \code{Observable} by plugging in these two functions, obtaining the following:

\hmint |%?a :: (Either Error (Maybe a) -> IO ()) -> IO Subscription |

This will have many implications on our definition of \code{Observable}, as moving away from an implementation based on the continuation monad will mean giving up on free class instances for \code{Observable} such as \code{Functor, Applicative, Monad}. This will not be a problem as, as we will see, we will be able to easily provide such instances by ourselves. 
